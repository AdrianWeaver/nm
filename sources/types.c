...
char get_symbol_type_64lsb(t_mem *file, Elf64_Sym *symbol, t_symbol *sym)
{
	...
	(void)sym; //kept to explain how types really work during evaluation.
#if SHOW_TYPE_DETAILS
	static int i;
	...
	if (symbol_bind >= STB_LOOS && symbol_type <= STB_HIOS)
		return ('u');
	if (executable_section && allocated_section &&
			shdr->sh_type == SHT_PROGBITS && (symbol_type == STT_FUNC || symbol_type == STT_NOTYPE))
	{
		if (symbol_bind == STB_LOCAL)
			return ('t');
		return ('T');
...
char get_symbol_type_32lsb(t_mem *file, Elf32_Sym *symbol, t_symbol *sym)
{
	...
	Elf32_Shdr *shdr = &shdr_table[symbol->st_shndx];

	unsigned char	symbol_type = ELF32_ST_TYPE(symbol->st_info);
	unsigned char	symbol_bind = ELF32_ST_BIND(symbol->st_info);
	uint32_t		executable_section = shdr->sh_flags & SHF_EXECINSTR;
	uint32_t		writable_section = shdr->sh_flags & SHF_WRITE;
	uint32_t		allocated_section = shdr->sh_flags & SHF_ALLOC;
	uint32_t		mergeable_section = shdr->sh_flags & SHF_MERGE;
...
char get_symbol_type_64msb(t_mem *file, Elf64_Sym *symbol, t_symbol *sym)
{
	...
	Elf64_Shdr *shdr = &shdr_table[rev16(symbol->st_shndx)];

	unsigned char	symbol_type = ELF64_ST_TYPE(symbol->st_info);
	unsigned char	symbol_bind = ELF64_ST_BIND(symbol->st_info);
	uint64_t		executable_section = rev64(shdr->sh_flags) & SHF_EXECINSTR;
	uint64_t		writable_section = rev64(shdr->sh_flags) & SHF_WRITE;
	uint64_t		allocated_section = rev64(shdr->sh_flags) & SHF_ALLOC;
	uint64_t		mergeable_section = rev64(shdr->sh_flags) & SHF_MERGE;
	...
	i++;
#endif
	if (rev16(symbol->st_shndx) == SHN_UNDEF)
	{
		if (symbol_bind == STB_WEAK)
		{
			if (symbol_type == STT_OBJECT)
	...
	if (rev16(symbol->st_shndx) == SHN_ABS)
		return ('A');
	if (rev16(symbol->st_shndx) == SHN_COMMON)
	{
		if (symbol_bind == STB_LOCAL)
			return ('c');
		return ('C');
	...
	if (symbol_bind >= STB_LOOS && symbol_type <= STB_HIOS)
		return ('u');
	if (executable_section && allocated_section &&
			rev32(shdr->sh_type) == SHT_PROGBITS && (symbol_type == STT_FUNC || symbol_type == STT_NOTYPE))
	{
		if (symbol_bind == STB_LOCAL)
			return ('t');
		return ('T');
	...
	if (writable_section && allocated_section && !executable_section && !mergeable_section)
	{
		if (rev32(shdr->sh_type) == SHT_NOBITS)
		{
			if (symbol_bind == STB_LOCAL)
				return ('b');
			return ('B');
	...
	if (rev32(shdr->sh_type) == SHT_NOBITS && allocated_section)
	{
		if (symbol_bind == STB_LOCAL)
			return ('s');
		return ('S');
	...
	//GNU SPECIFIC UNTESTED
	if ((rev32(shdr->sh_type) == SHT_REL || rev32(shdr->sh_type) == SHT_RELA)
			&& allocated_section && symbol_type == STT_FUNC)
	{
		if (symbol_bind == STB_LOCAL)
			return ('i');
...
char get_symbol_type_32msb(t_mem *file, Elf32_Sym *symbol, t_symbol *sym)
{
	...
	Elf32_Shdr *shdr = &shdr_table[rev16(symbol->st_shndx)];

	unsigned char	symbol_type = ELF32_ST_TYPE(symbol->st_info);
	unsigned char	symbol_bind = ELF32_ST_BIND(symbol->st_info);
	uint32_t		executable_section = rev32(shdr->sh_flags) & SHF_EXECINSTR;
	uint32_t		writable_section = rev32(shdr->sh_flags) & SHF_WRITE;
	uint32_t		allocated_section = rev32(shdr->sh_flags) & SHF_ALLOC;
	uint32_t		mergeable_section = rev32(shdr->sh_flags) & SHF_MERGE;
